...
# // English: Define input files (from Step 1 and the raw data) and the output file for this step.
# // 中文: 定义输入文件 (来自步骤1和原始数据) 以及此步骤的输出文件。
INPUT_RDS_STEP1 <- here("dataset", "clean_R", "Step1_Resilience_Flags_Corrected.rds")
INPUT_SAV_FILE <- here("dataset", "meta", "CY08MSP_STU_QQQ.SAV")
OUTPUT_RDS_FILE <- here("dataset", "clean_R", "Step2_Merged_Cleaned_for_Imputation.rds")

# --- 3. Define All Predictor Variables ---
# --- 3. 定义所有预测变量 ---

# // English: This list contains all the student, teacher, and school variables from your research plan.
# // 中文: 这个列表包含了你研究计划中所有的学生、教师和学校层面的变量。
predictor_cols <- c(
  "CNTSTUID", # // Student ID is essential for merging / 学生ID是合并所必需的
...
# // English: Load the foundational dataset created in Step 1.
# // 中文: 加载步骤1中创建的基础数据集。
print("Loading foundational data from Step 1...")
data_step1 <- readRDS(INPUT_RDS_STEP1)

# // English: Load the predictor variables from the raw PISA file.
# // 中文: 从原始PISA文件中加载预测变量。
...
# // English: Merge the two datasets using the unique student ID.
# // 中文: 使用唯一的学生ID合并两个数据集。
...
# // English: This list defines the specific "invalid" codes for each variable, based on the PISA codebook.
# // 中文: 这个列表根据PISA代码手册为每个变量定义了特定的“无效”代码。
...
data_cleaned <- merged_data %>%
  mutate(
    ...
    # --- Part B: Clean OCOD3 and Engineer the New Feature (The Fix) ---

    # 1. First, handle the special invalid codes for OCOD3 specifically.
    ...
    # 2. Convert the cleaned column to numeric. This will safely turn non-numeric text into NA.
    OCOD3_numeric = as.numeric(OCOD3_cleaned),

    # 3. Now, you can safely perform the calculation on the fully numeric column.
...
# --- 6. Diagnose Structural Missingness (Guidance for Next Step) ---
# --- 6. 诊断结构性缺失 (为下一步提供指导) ---

# // This code calculates the missing percentage for each variable by country.
# // It helps to identify which countries may not have administered certain modules.
# // 这段代码计算了每个变量在各个国家的缺失百分比。
# // 它可以帮助识别哪些国家可能没有实施某些问卷模块。

...
for (i in 1:10) {
  ...
  #     )
  #   )
  resilience_score_name <- paste0("RESILIENCE_SCORE_PV", i)
  final_data <- final_data %>%
    mutate(
      !!resilience_score_name := if_else(
        !is.na(!!sym(threshold_col_name)),
        (!!sym(pv_name) - !!sym(threshold_col_name)) / 100,
        NA_real_
      )
    )
...
final_data_cleaned <- final_data %>%
  select(
    ...
    starts_with("PV"),
    starts_with("RESILIENCE_SCORE_PV"),

...
# --- 4. PRE-PROCESSING & SCHOOL FILTERING ---
print("Pre-processing and filtering schools by size...")

# --- NEW: Define the minimum number of students required for a school to be included ---
# You can change this value to experiment (e.g., 5, 10, 15)
min_students_per_school <- 0

list_of_engineered_datasets <- lapply(list_of_engineered_datasets, function(df) {

  # --- NEW: Add diagnostic printout before filtering ---
  original_students <- nrow(df)
  original_schools <- length(unique(df$CNTSCHID))

  filtered_df <- df %>%
    # --- NEW: Group by school and count students in each school ---
    group_by(CNTSCHID) %>%
    mutate(n_students_in_school = n()) %>%
    ungroup() %>%

    # --- NEW: Filter out schools with fewer students than the minimum threshold ---
    filter(n_students_in_school >= min_students_per_school)

  # --- NEW: Add diagnostic printout after filtering ---
  filtered_students <- nrow(filtered_df)
  filtered_schools <- length(unique(filtered_df$CNTSCHID))

  print(paste0("Filtered from ", original_schools, " schools and ", original_students, " students"))
  print(paste0(" down to ", filtered_schools, " schools and ", filtered_students, " students (min ", min_students_per_school, " per school)."))

  # --- Existing pre-processing steps applied to the filtered data ---
  filtered_df %>%
    mutate(
      FEMALE = as.factor(if_else(ST004D01T == 1, 1, 0)),
      IMMIG = as.factor(IMMIG),
    ...
    # We no longer need the temporary count column
    select(-n_students_in_school) %>%
    arrange(CNTSCHID)
})
print("Pre-processing and filtering complete.")
...
original_model_2_predictors <- "ESCS +  FEMALE + IMMIG +
   EXPECEDU +
   REPEAT + MISSSC + EXERPRAC +
   STUDYHMW + WORKPAY + WORKHOME + INFOSEEK + EXPOFA +
   EXPO21ST + CREATAS + STRESAGR + BULLIED"
...
model_0_formula <- as.formula("~ Math_Disposition")
results_model_2 <- run_and_pool_with_mitools(bifie_data_list, model_0_formula, "Model_Full_with_LVs", pv_names)
